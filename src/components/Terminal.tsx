...
// --- API routes terminal ---
export function TerminalWindow({ title, routes }: { title: string; routes: Route[] }) {
  return (
    <TerminalShell title="terminal">
      <div className="p-3 sm:p-4 lg:p-5 font-['JetBrains_Mono'] text-[10px] sm:text-xs lg:text-sm space-y-1.5 overflow-x-auto">
        <div>
          ...
          <span className="text-gray-400">node dist/server.js</span>
        </div>
        <div className="py-0.5" />
        <div className="text-gray-200 font-medium">{title}</div>
        <div className="text-gray-600">{routes.length} routes registered</div>
        <div className="py-0.5" />
        {routes.map((route) => (

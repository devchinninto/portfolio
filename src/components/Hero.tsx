...
function WindowControls() {
  return (
    <div className="flex gap-2 mb-6">
      <div className="w-3.5 h-3.5 rounded-full bg-gradient-to-br from-red-500 to-red-600" />
      <div className="w-3.5 h-3.5 rounded-full bg-gradient-to-br from-yellow-500 to-yellow-600" />
...
function HeroButton({
  ...
  const base = 'px-8 py-4 rounded-xl font-medium transition-all duration-300'
...
function CodeEditorCard() {
  ...
  return (
    <div className="glass-code-card rounded-[20px] p-7">
      <WindowControls />
      <pre className="font-['JetBrains_Mono'] text-[14px] leading-relaxed">
        <code>
...
export function Hero() {
  const { name, role, bio, buttons } = HERO_CONTENT
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          >
            <div className="space-y-4">
              <h1 className="text-[56px] font-bold leading-tight text-gradient-white">
                {name}
              </h1>
              <h2 className="text-[28px] text-[#00FF88]">{role}</h2>
              <p className="text-[#c7d2fe] text-base leading-relaxed max-w-xl">
                {bio}
              </p>
            </div>
